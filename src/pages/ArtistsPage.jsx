...
function ArtistGroupHeading({ id, title, first }) {
  return (
    <header
      className={`border-b border-border/80 pb-3 sm:pb-4 ${first ? 'mb-6 sm:mb-7' : 'mb-7 sm:mb-8'} ${!first ? 'mt-12 border-t border-border/60 pt-10 sm:mt-16 sm:pt-14' : ''}`}
    >
      <h2
        id={id}
      ...
      <span
        className="mt-2 inline-block h-px w-10 rounded-full bg-gradient-to-r from-studio-gold/70 via-studio-gold/35 to-transparent sm:w-12"
        aria-hidden="true"
      />
    </header>
  )
...
function ArtistCard({ a, i, reduce }) {
  return (
    <motion.li
    ...
    >
      <div className="aspect-[4/5] overflow-hidden bg-surface">
        <img
          ...
          style={
            (a.profileObjectFit ?? 'cover') !== 'contain' && a.profileObjectPosition
      ...
      <div className="flex min-h-0 flex-1 flex-col p-6">
        ...
        <h3 className="mt-3 font-display text-2xl tracking-wide text-zinc-100">{a.name}</h3>
        {a.status === 'temporary' && a.availabilityLabel && (
          <p className="mt-2 text-xs text-amber-400/80">{a.availabilityLabel}</p>
        )}
        {a.status === 'temporary' && !a.availabilityLabel && a.availableFrom && a.availableTo && (
          <p className="mt-2 text-xs text-amber-400/80">Available {formatRange(a.availableFrom, a.availableTo)}</p>
        )}
        {a.status === 'temporary' && !a.availabilityLabel && (!a.availableFrom || !a.availableTo) && (
          <p className="mt-2 text-xs text-amber-400/80">Guest — visit dates announced on socials.</p>
        )}
        <div
...
export default function ArtistsPage() {
  ...
  const residents = artists.filter((a) => a.status === 'permanent')
  const guests = artists.filter((a) => a.status === 'temporary')
  ...
  return (
    <>
      ...
      <section className="pt-4 pb-16 sm:pt-5 sm:pb-20" aria-label="Artist profiles">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div>
            <ArtistGroupHeading id="resident-artists-heading" title="Resident artists" first />
            <ul
              className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3"
              aria-labelledby="resident-artists-heading"
            >
              {residents.map((a, i) => (
                <ArtistCard key={a.id} a={a} i={i} reduce={reduce} />
              ))}
            </ul>
          </div>

          <div>
            <ArtistGroupHeading id="guest-artists-heading" title="Guest artists" first={false} />
            <ul
              className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3"
              aria-labelledby="guest-artists-heading"
            >
              {guests.map((a, i) => (
                <ArtistCard key={a.id} a={a} i={i} reduce={reduce} />
              ))}
