/** Studio contact email — used in UI, mailto links, and README */
export const STUDIO_EMAIL = 'onebloodtattoostudio@gmail.com'

/** Studio phone — footer `tel:` link; E.164 for href */
export const STUDIO_PHONE_DISPLAY = '469-438-9491'
export const STUDIO_PHONE_TEL = '+14694389491'

/**
 * Brand mark — place your file at `public/logo.png`.
 * Used for the browser tab icon, header, and footer.
 */
export const STUDIO_LOGO_SRC = '/logo.png'

/** Physical address — footer, booking sidebar */
export const STUDIO_ADDRESS = {
  line1: '4101 Bryan St, Suite 120',
  line2: 'Dallas, Texas 75204',
}

/** Google Maps search for the studio (open in new tab from footer) */
export const STUDIO_MAPS_URL =
  'https://www.google.com/maps/search/?api=1&query=4101+Bryan+St+Suite+120%2C+Dallas%2C+TX+75204'

/** Walk-in window (Mon–Sun, 12–8); bookings use artist-specific timing */
export const STUDIO_HOURS_NOTE =
  'Walk-ins are welcome during walk-in hours. Booked sessions follow your artist’s schedule — confirmations are sent by email.'
export const STUDIO_WALKIN_HOURS = 'Mon — Sun · 12 PM — 8 PM'

/** Merch page + nav links — set `true` when the line is ready to show publicly */
export const FEATURE_MERCH = false

/**
 * Studio showreel (Home page). Place the file at `public/gallery/showreel.mov`.
 */
export const STUDIO_SHOWREEL_VIDEO_SRC = '/gallery/showreel.mov'
